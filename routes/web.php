...
Route::post( '/logintenants', function (Request $request) {
...
try {
     ...
     $user = Data::where('rti_company', '=', $tenantid)->first();
     if (!$user) {
        return response()->json(['success'=>false, 'message' => 'Login Fail, please check email id']);
     }
     if (!Hash::check($password, $user->rti_password)) {
...
Route::post( '/addtenant', function (Request $request) {
    ...
    $rules = array (
            ...
            'faxno' => 'required',
            'buldingname' => 'required',
            'buildingfloor' =>'required',
            'buildingunit' => 'required',
    );
    $validator = Validator::make ( Input::all (), $rules );
    if ($validator->fails ())
        return Response::json ( array (
...
try {
        $password = Hash::make($request->input('companyname'));
        ...
        $id = DB::table('r_tenants_informations')->insertGetId($data);
        $data = Data::where('rti_tenantid', $id)->first();
...
Route::post( '/editTenantDetails', function (Request $request) {
    
    $rules = array (
            'companyname' => 'required',
            'rti_tenantid' => 'required',
            'ownername' => 'required',
            'telephone' => 'required',
            'emailaddress' => 'required',
            'faxno' => 'required',
    ...
    if ($validator->fails ())
      ...
      $data = 
    Data::where('rti_tenantid', $request->rti_tenantid)->update([
            'rti_company' => $request->companyname,
            'rti_owner' => $request->ownername,
            'rti_telephone' => $request->telephone,
            'rti_emailaddress' => $request->emailaddress,
            'rti_faxno' => $request->faxno,
            'rti_building' => $request->buldingname,
            'rti_floor' => $request->buildingfloor,
            'rti_unit' => $request->buildingunit
     ]);
    $data = Data::where('rti_tenantid', $request->rti_tenantid)->first();
        return response ()->json ( $data );
    }
} );
Route::get('/', function () {
...
Route::get('/admin/tenantlist', function () {

$data = Data::all ();
    return view('pages/admin/tenantlist')->withData ( $data );
})->name('tenantlist');
